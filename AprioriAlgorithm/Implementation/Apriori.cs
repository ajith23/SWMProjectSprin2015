namespace AprioriAlgorithm
{
    ...
    public class Apriori : IApriori
    {
        ...
        private List<Item> GetL1FrequentItems(double minSupport, IEnumerable<string> items, IEnumerable<string> transactions)
        {
            ...
            frequentItemsL1 = frequentItemsL1.Where(s => !string.IsNullOrWhiteSpace(s.Name)).Distinct().ToList();
        ...
        {
            var length = firstItem.Split('|').Count();
            ...
            {
                string firstSubString = firstItem.Split('|')[length - 1];
                string secondSubString = secondItem.Split('|')[length - 1];

                if (firstSubString == secondSubString)
                {
                    return firstItem + "|" +secondItem.Split('|')[length - 1];
                }
